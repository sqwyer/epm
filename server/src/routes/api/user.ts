        ...
        if (project) {
            if (req.body.email) {
                try {
                    ...
                    if (user) {
                        const invite = project.pending_members.find(
                            (self) => self.id == user.id
                        )
                        if (invite) {
                            project.pending_members.splice(
                                project.pending_members.indexOf(invite, 1)
                            )
                            user.pending_projects.splice(
                                user.pending_projects.indexOf(project.id, 1)
                            )
                ...
                try {
                    ...
                    if (user) {
                        if (!project.pending_members.includes(user.id)) {
                            ...
                            project.pending_members.splice(
                                project.pending_members.indexOf(user.id, 1)
                            )
                            user.pending_projects.splice(
                                user.pending_projects.indexOf(project.id, 1)
                            )
