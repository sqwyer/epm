...
class User {
    public static async getProjects(id: string): Promise<ProjectType[]> {
        return new Promise((resolve: Function, reject: Function) => {
            UserModel.findOne({id}, async (err: any, user: HydratedDocument<UserType>) => {
                if(err) reject(err)
                    ...
                    for(let i = 0; i < user.projects.length; i++) {
                        try {
                            const project: HydratedDocument<ProjectType> = await ProjectModel.findById(user.projects[i]).exec();
